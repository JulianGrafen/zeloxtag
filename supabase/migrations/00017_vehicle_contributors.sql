-- =============================================================================
-- ZeloxTag · Vehicle contributors (Schrauber)
-- Migration: 00017_vehicle_contributors
-- =============================================================================
-- Owners can invite mechanics ("Schrauber") who may add invoice / repair /
-- service documents to the vehicle twin. Ownership of documents.user_id stays
-- with the vehicle owner (trigger from 00012). created_by records the writer.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- documents.created_by — who uploaded (owner or contributor)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Helper: active schrauber on a vehicle
-- -----------------------------------------------------------------------------
...
-- Invitee may accept their own pending invite (token lookup uses service role
-- in the app; this covers session-client accept paths).
create policy "vehicle_contributors_update_self_accept" on public.vehicle_contributors
  for update
  to authenticated
  using (
    status = 'invited'
...
-- -----------------------------------------------------------------------------
-- vehicles: contributors may SELECT (make/model for dashboard)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- tags: contributors may SELECT linked tags
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- documents: contributors read + insert invoices; update/delete own uploads
-- -----------------------------------------------------------------------------
...
create policy "documents_select_contributor" on public.documents
  for select
  to authenticated
  using (public.is_active_vehicle_contributor(vehicle_id));
