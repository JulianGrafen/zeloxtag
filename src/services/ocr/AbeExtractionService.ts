  ...
  async extractVehiclesFromDocument(
    ...
    const raw = await this.runWizardStep<AbeWizardVehiclesExtraction>(
      ...
      [
        ...
        "CRITICAL — model is the exact vehicle model at the START of the Auflagen cell for THIS row.",
        "After the model name, the same Auflagen cell lists short condition codes (numeric/alphanumeric).",
        "NEVER use the Fahrzeugtyp column (left) or the Verkaufsbezeichnung section header above groups.",
        "Column mapping:",
        "- model: leading vehicle model text in the Auflagen column for this row only.",
        ...
        "- auflagenCodes: only the short codes after the model in Auflagen — never the model text.",
      ...
      [
        ...
        "For each row, read the Auflagen cell from the beginning: first the exact model, then short codes.",
        "Ignore the Fahrzeugtyp column for model. Ignore Verkaufsbezeichnung headers for model.",
