      ...
      const extractRows = async (isRetry: boolean) => {
        const raw = await this.runSnippetStep(
          ...
          [
            ...
            "Cropped photos usually show 6–7 columns: Fahrzeugtyp | Betriebserlaubnis / EG-BE | kW | Reifen | Reifenbezogene Auflagen | Auflagen und Hinweise (rightmost).",
            ...
            "Column mapping for the cropped table:",
            ...
            "- reifenbezogeneAuflagenCodes: left Auflagen column (next to Reifen) — every short Kürzel from that cell.",
          ...
          [
            ...
            "Typical cropped columns: Fahrzeugtyp | Betriebserlaubnis | kW | Reifen | Reifenbezogene Auflagen | Auflagen und Hinweise (rechts). Model name is the header above, not a column.",
          ],
          ABE_HUNT_VEHICLE_JSON_SCHEMA,
    ...
    try {
      ...
      const raw = await this.runSnippetStepWithPdfHybrid(
        ...
        [
          ...
          'Map "Auftraggeber" to abeHolder when no separate Inhaber der ABE label is shown.',
          "Verwendungsbereich / Fahrzeugtabelle: copy ONLY printed text. Model header above the table → verkaufsbezeichnung; FIRST data column → fahrzeugtyp (never null when visible, never invent); Betriebserlaubnis / EG-BE → typeApproval; Reifen → tireSizes. Cropped photos are often 6–7 columns: include BOTH Auflagen columns (reifenbezogene + rightmost Auflagen und Hinweise).",
          ...
          "Each row's reifenbezogeneAuflagenCodes and auflagenUndHinweiseCodes must come ONLY from that row's own Auflagen cells on the same table line — never merge codes from other rows or page headers.",
          "Put reifenbezogene Kürzel in reifenbezogeneAuflagenCodes and RIGHTMOST 'Auflagen und Hinweise' Kürzel in auflagenUndHinweiseCodes — never in the top-level auflagenCodes field.",
          "When extracting vehicleMatches: never put Auflagen-Kürzel in fahrzeugtyp or tireSizes.",
